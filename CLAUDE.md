...
- **`Инициализация` (init) — на Konda, собирается через `konda build`** (`основа.конда`
  + C-шим `конда_init.h`). Её `Makefile` зовёт **фронтенд системы сборки Konda**
  (`../ТранспиляторКонда/сборка/вывод/сборка.elf`) с манифестом `Konda.toml`
  (`аллокатор = libc`, `стдлиб = нет`), а НЕ транспилятор напрямую (переведено с
