...
**Вывод — ЕДИНЫЙ поток.** `основа.c` в начале `main` сливает `stderr`→`stdout`
(`dup2(STDOUT_FILENO, STDERR_FILENO)`) и делает stdout построчным
(`setvbuf(_IOLBF)`), плюс `fflush(NULL)` перед каждым `system()` (обёртка
`выполнить_систему`). Итог: ВЕСЬ вывод — и сообщения оркестратора, и stdout+stderr
всех потомков (`make`/`meson`/`ninja`/gcc, под-скрипты образа) — идёт в ОДНО место,
по порядку, видимый целиком и в терминале, и в логе CI. Потомки наследуют слитый
дескриптор, поэтому `2>&1` / `2>log.txt` дописывать НИГДЕ не нужно (и НЕЛЬЗЯ — это
сломает единый поток). Без этого при stdout-pipe (CI, не терминал) glibc полностью
буферизует наши сообщения → они всплывают пачкой в конце, а ошибки детей идут в
реальном времени → лог перемешан, заголовок «Собираю: X» отрывается от ошибки X
(был симптом: ошибка weston не читалась в логе CI).
