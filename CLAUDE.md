...
- **`Инициализация` (init) — на ЧИСТОМ Konda, собирается через `konda build`**
  ...
  **gcc ≥ 15** (его Makefile гейтит `gcc >= 15`: musttail/TCO + `-std=gnu23`; раннер
  ...
  ⚠ **stdlib подключён (`стдлиб = да`), но зависимости от `libkonda_std.so` у
  init.elf ПОКА НЕТ:** автолинк ставит `.so` на линию `cc`, но init не использует
  ни одного stdlib-символа → `--as-needed` (дефолт) отбрасывает `.so` из NEEDED
  (проверено: NEEDED = только `libc.so.6`). `libkonda_std.so` сам тянет ТОЛЬКО
  libc (у `.so` аллокатор выбирает потребитель — jemalloc НЕ появляется). **Как
  только init начнёт вызывать функцию stdlib**, `.so` станет NEEDED и ОБЯЗАН
  лежать в `rootfs/lib64` (иначе PID 1 не стартует → kernel panic); `добавить_
  программу.sh` притянет его по `ldd` автоматически (досев как для прочих deps).
