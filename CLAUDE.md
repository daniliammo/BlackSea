...
- **`Инициализация` (init) — на ЧИСТОМ Konda, собирается через `konda build`**
  (`основа.конда`; C-шима `конда_init.h` БОЛЬШЕ НЕТ — удалён). Запуск busybox sh
  делает **встроенная `заменить_образ`** (ТранспиляторКонда §114): `execvp` без
  сырого `char**` (язык сам строит NUL-терминированный argv из среза строк) →
  доверенный C-шим для exec не нужен. Её `Makefile` зовёт **фронтенд системы
  сборки Konda** (`../ТранспиляторКонда/сборка/вывод/сборка.elf`) с манифестом
  `Konda.toml` (`аллокатор = libc`, **`стдлиб = да`** — stdlib подключён,
  init.c включает `libkonda_std.h`), а НЕ транспилятор напрямую (переведено с
  согласия пользователя; раньше — прямой вызов `ТранспиляторКонда --аллокатор=libc
  --без-стандартной-библиотеки`). Выход — `вывод/init.elf` (имя из `[проект].имя`),
  копируется в `Собранное/init`. ⚠ **БУТСТРАП-ПОРЯДОК
  (СДЕЛАНО):** init зависит от СОБРАННОГО транспилятора И фронтенда, а `scandir+alphasort`
  ставит `Инициализация` (кир. «И») РАНЬШЕ `ТранспиляторКонда` (кир. «Т») → оркестратор
  собрал бы init прежде транспилятора и упал («ТранспиляторКонда: No such file»,
  код 127). `основа.c` теперь **хойстит `ТранспиляторКонда`** — строит его ПЕРВЫМ,
  ДО общего цикла (вынесено в `собрать_программу()`), а в цикле пропускает, чтобы
  не собирать дважды. Его `make` (цель `all`) собирает и **фронтенд `konda build`**
  (prereq stdlib) → к сборке init оба на месте. **CI (`ci.yml`) обязан**
  инициализировать субмодуль
  `ТранспиляторКонда` (иначе «нет Makefile» → init не соберётся) И ставить
  **gcc ≥ 15** (его Makefile гейтит `gcc >= 15`: musttail/TCO + `-std=gnu23`; раннер
  даёт gcc 13.3) + **`libjemalloc2`** — теперь для **самого фронтенда** `сборка.elf`
  (он собран транспилятором с ДЕФОЛТНЫМ аллокатором → линкует `libjemalloc.so.2`;
  init.elf же — с `аллокатор=libc`, jemalloc НЕ тянет). ✅ **РЕШЕНО:** init.elf на
  Konda раньше в рантайме тянул `libjemalloc.so.2`, которого нет в `rootfs/lib64` →
  в образе не стартовал; переход на `аллокатор=libc` убрал эту зависимость (init.elf
  линкует только `libc.so.6`, `readelf -d` подтверждает).
  ✅ **init РЕАЛЬНО зависит от stdlib** (`стдлиб = да`): диагностику печатает через
  модуль `поток` (БуферВывода) → использует символы `libkonda_std.so` →
  `NEEDED = libkonda_std.so + libc.so.6` (проверено `readelf -d`). `libkonda_std.so`
  сам тянет ТОЛЬКО libc (у `.so` аллокатор выбирает потребитель — jemalloc НЕ
  появляется). `.so` ОБЯЗАН лежать в `rootfs/lib64` (иначе PID 1 не стартует →
  kernel panic): `добавить_программу.sh` притягивает его по `ldd` автоматически
  (init.elf RUNPATH → `stdlib/вывод`), `создать_образ.sh` `ldconfig` кладёт в
  `ld.so.cache`. ⚠ Требует SONAME на `.so` (ТранспиляторКонда §116): без него в
  `NEEDED` попал бы АБСОЛЮТНЫЙ build-путь, которого в образе нет. `int→строка` в
  stdlib нет → сообщение с кодом ошибки осталось на libc `printf`.
...
### Сборка без предупреждений (`-Werror`) — для ВСЕГО нашего кода
- **ПОЛИТИКА: наш код собирается clean, предупреждение компилятора = ошибка сборки
  (`-Werror`).** Появилось предупреждение — ИСПРАВЛЯЕМ в исходнике, а не подавляем.
  Это та же линия, что «только свежий тулчейн» (свежий компилятор строже — ловит
  больше). Проверять на **gcc и clang** (оба свежие).
- **Что покрыто (наш код):**
  - `Сборка/основа.c` (оркестратор) — `-Werror` в `Сборка/Makefile`.
  - `Программы/Инициализация` (init) — теперь **на Konda**, собирается транспилятором
    со строгой clean-сборкой выхода по умолчанию (`-Wall -Wextra -Werror`, см.
    ТранспиляторКонда §65). Обычный C-init заменён (см. «Загрузка / init»).
  - `Программы/ТранспиляторКонда` — сам транспилятор `-Werror`; его ВЫХОД (любая
    `.конда`-программа, включая init и stdlib) — тоже строго (§65). Escape-hatch
    `--разрешить-предупреждения` — только для осознанных предупреждений
    пользовательского C (pass-through printf, литерал UINT64_MAX).
  - `Программы/KondaFromClang` (kfc) — **Python**, `-Werror` неприменим; чистота —
    линтером (ruff/pyflakes), отдельно.
- **Что ИСКЛЮЧЕНО (чужой upstream — не наш код, `-Werror` не навязываем):** `Ядро`
  (torvalds/linux), `busybox`, `weston` и его subproject'ы (wayland-стек). Их
  предупреждения — забота upstream; ломать их сборку `-Werror`'ом нельзя.
- `Программы/СтандартнаяБиблиотекаКонда` — по указанию не трогаем (см. «Не трогать
  в коммитах»); как Konda-код её выход всё равно строг (§65) при сборке.
