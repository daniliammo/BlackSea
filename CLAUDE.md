...
- **Следствие для CI (TODO):** джоб `image` в `ci.yml` пакует закоммиченный
  rootfs, который теперь БЕЗ бинарников → образ выйдет пустым. Чтобы CI снова
  давал рабочий образ, джоб `image` должен перед `создать_образ.sh` собрать в
  rootfs busybox (`busybox.config`→build→`make CONFIG_PREFIX=rootfs install`),
  `Инициализация`→`sbin/init`, weston (`собрать-weston.sh`) и связку glibc.
  Локальная сборка (`cd Сборка && make`) не затронута — она это и делает.
