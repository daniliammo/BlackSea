...
- **`Инициализация` (init) — теперь на Konda** (`основа.конда` + C-шим
  ...
  init зависит от СОБРАННОГО транспилятора, а `scandir+alphasort` ставит
  `Инициализация` (кир. «И») РАНЬШЕ `ТранспиляторКонда` (кир. «Т») → без правки
  оркестратор соберёт init прежде транспилятора и упадёт. НУЖНО: `основа.c`
  оркестратора должен строить `ТранспиляторКонда` ПЕРВЫМ (хойст, как особый
  случай), потом остальные. *(На момент порта init эта правка оркестратора —
  открытый следующий шаг; см. коммит порта init.)*
