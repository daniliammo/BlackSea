...
- **Симптом «ядро перебирает `/sbin/init → /etc/init → /bin/init`»** (или паника
  ...
  `создать_образ.sh` СТРАХУЕТ: `/sbin/init` — настоящий ELF (не симлинк) + загрузчик
  и libc на месте, иначе стоп. **Инспекция образа БЕЗ рута**: `dd` вырезать
  ext4-раздел (offset из `fdisk -l img`) → `debugfs -R "stat/dump …"`.
