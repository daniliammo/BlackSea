...
- **`Инициализация` (init) — теперь на Konda** (`основа.конда` + C-шим
  `конда_init.h`), её `Makefile` зовёт **транспилятор Konda**
  (`../ТранспиляторКонда/Собранное/ТранспиляторКонда`). ⚠ **БУТСТРАП-ПОРЯДОК
  (СДЕЛАНО):** init зависит от СОБРАННОГО транспилятора, а `scandir+alphasort`
  ...
  не собирать дважды. **CI (`ci.yml`) обязан** инициализировать субмодуль
  ...
  **gcc ≥ 15** (его Makefile гейтит `gcc >= 15`: musttail/TCO + `-std=gnu23`; раннер
  даёт gcc 13.3) + **`libjemalloc2`** (сгенерированный init.elf линкуется
  `-l:libjemalloc.so.2` — дефолтный аллокатор транспилятора). ⚠ ОТКРЫТО: init.elf
  в рантайме тянет `libjemalloc.so.2`, а в `rootfs/lib64` его никто не кладёт
  (`основа.c` копирует init, но НЕ прогоняет `добавить_программу.sh` для его
  зависимостей) → в образе init на Konda пока не стартует; досев jemalloc/libc для
  init — следующий шаг порта.
